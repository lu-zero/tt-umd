...
void PCIDevice::configure_tlb(const uint32_t tlb_index, const tlb_data &tlb_config) {
    ...
    // Use 32-bit stores throughout: on aarch64, PCIe WC regions require natural alignment, and
    // Blackhole registers are 12 bytes apart (index * 12), making odd indices 4-byte aligned but
    // NOT 8-byte aligned. A 64-bit store to a non-8-byte-aligned WC address causes SIGBUS.
